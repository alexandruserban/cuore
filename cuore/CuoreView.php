...
class CuoreView {
	...
	function cache($path, $variables = array(), $force_cache = false) {
        ...
        $tmp_cache_file = self::$tmp_cache_dir . $tmp_prefix .'_'.md5(filesize($full_path) . print_r($variables, true));
        $now = time();
        if (!file_exists($tmp_cache_file) || ((self::$cache_time == -1 || $force_cache || (($now - filemtime($tmp_cache_file)) > self::$cache_time)) && self::$cache_time) ) {
            ...
            $old_tmp_cache_file = array_pop(glob(self::$tmp_cache_dir . $tmp_prefix . '_*'));
